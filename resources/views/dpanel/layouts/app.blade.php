...
<body class="w-full flex flex-no-wrap bg-gray-100">
    ...
    <x-dpanel::sidebar.container name="DD Admin">
        ...
        <x-dpanel::sidebar.item name="Categories" icon="bx-category-alt" url="{{ route(config('dpanel.prefix') . '.category.index') }}"
            isActive="{{ request()->segment(2) == 'category' }}" />

        <x-dpanel::sidebar.item name="Campaigns" icon="bx-donate-heart" url="{{ route(config('dpanel.prefix') . '.campaign.index') }}"
